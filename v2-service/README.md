...
Можно подключить генерацию раздела `hierarchy` через LLM (Ollama): по терминам и
функциям модель предлагает связи. Фича **выключена по умолчанию** и не тянет
тяжёлый стек в обычный образ — включается отдельным оверлеем:

```bash
# собрать образ с экстрой [ai], поднять Ollama, включить фичу:
docker compose -f docker-compose.yml -f docker-compose.ai.yml up --build

# один раз скачать модель (вес — гигабайты):
docker compose -f docker-compose.yml -f docker-compose.ai.yml exec ollama ollama pull llama3
```

Оверлей [docker-compose.ai.yml](docker-compose.ai.yml) ставит `INSTALL_AI=true`
(образ с `[ai]`), выставляет `AI_ENABLED=true` / `OLLAMA_URL`, поднимает `ollama`.
Когда включено, на странице проекта появляется кнопка **«Связи (AI)»**: результат —
предложенные связи + готовый к вставке фрагмент `.ontol` (файл не меняется
автоматически). Запрос рендерится фоновым воркером.

| Переменная | Назначение | Дефолт |
|------------|-----------|--------|
| `AI_ENABLED` | включить фичу | `false` |
| `OLLAMA_URL` | адрес Ollama | `http://ollama:11434` |
| `AI_MODEL` | модель по умолчанию | `llama3` |

## Замечания

- В обычном образе AI-стек (langchain) не ставится (ядро ontol — без экстры
  `[ai]`); он подтягивается только AI-оверлеем (см. выше).
